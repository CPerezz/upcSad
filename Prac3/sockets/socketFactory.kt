...
class MySocket : Socket {
    //Custom Socket class created by encapsulation to handle with end of connection with the server scoket
    ...
    override fun close() {
        try {
            ...
            println("Error during closing connection with the server")
            //e.printStackTrace();
        }
    ...
    companion object {
        ...
        fun fromRute(adress: String, port: Int): MySocket? =
                try {
                    ...
                    println("Error connecting with server +$ex")
                    null
                }
